...
echo ./mvnw -B versions:set scm:checkin -DremoveSnapshot -DgenerateBackupPoms=false -Dmessage="\"prepare release ${tag}\""

# Update the versions to the next snapshot
echo ./mvnw -B versions:set scm:checkin -DnewVersion="${snapshot}" -Dmessage="updating versions to next development iteration ${snapshot}"